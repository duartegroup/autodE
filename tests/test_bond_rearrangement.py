...
def test_2b():
    ...
    assert rearr.get_fbonds_bbonds_2b(reac, prod, [], [[(0, 1), (1, 2)]], [], [], [(0, 2)], [], 0) == [rearr.BondRearrangement(breaking_bonds=[(0, 1), (1, 2)])]

    reac = Molecule(xyzs=[['C', 0, 0, 0], ['H', 0.6, 0, 0], ['H', 1.2, 0, 0]])
    prod = Molecule(xyzs=[['C', 0, 0, 0], ['H', 10, 0, 0], ['H', 20, 0, 0]])
    assert rearr.get_fbonds_bbonds_2b(reac, prod, [], [[(0, 1)], [(1, 2)]], [], [], [], [], 0) == [rearr.BondRearrangement(breaking_bonds=[(0, 1), (1, 2)])]


def test_1b1f():
    reac = Molecule(xyzs=[['C', 0, 0, 0], ['H', 0.6, 0, 0], ['H', 10, 0, 0]])
    prod = Molecule(xyzs=[['C', 0, 0, 0], ['H', 10, 0, 0], ['H', 10.6, 0, 0]])
    assert rearr.get_fbonds_bbonds_1b1f(reac, prod, [], [[(0, 1)]], [[(1, 2)]], [], [], [], 0) == [rearr.BondRearrangement(forming_bonds=[(1, 2)], breaking_bonds=[(0, 1)])]
...
def test_1b2f():
    ...
    assert rearr.get_fbonds_bbonds_1b2f(reac, prod, [], [[(0, 1)]], [[(0, 2)], [(1, 2)]], [], [], [], 0) == [rearr.BondRearrangement(forming_bonds=[(0, 2), (1, 2)], breaking_bonds=[(0, 1)])]
    ...
    assert rearr.get_fbonds_bbonds_1b2f(reac, prod, [], [], [[(0, 2), (1, 2)]], [], [], [(0, 1)], 0) == [rearr.BondRearrangement(forming_bonds=[(0, 2), (1, 2)], breaking_bonds=[(0, 1)])]


def test_2b1f():
    reac = Molecule(xyzs=[['H', 0, 0, 0], ['C', 0.6, 0, 0], ['O', 1.2, 0, 0]])
    prod = Molecule(xyzs=[['H', 0, 0, 0], ['C', 10, 0, 0], ['O', 0.6, 0, 0]])
    assert rearr.get_fbonds_bbonds_2b1f(reac, prod, [], [[(0, 1)], [(1, 2)]], [[(0, 2)]], [], [], [], 0) == [rearr.BondRearrangement(forming_bonds=[(0, 2)], breaking_bonds=[(0, 1), (1, 2)])]
