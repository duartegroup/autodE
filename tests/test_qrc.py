...
def test_hshift_displacement():
    ...
    assert np.max(np.linalg.norm(ts.coordinates - f_disp_mol.coordinates, axis=1)) > 0.5
    ...
    max_disp = np.max(np.linalg.norm(ts.coordinates - f_disp_mol.coordinates, axis=1))
