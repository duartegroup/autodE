...
def test_get_closest_species():
    ...
    assert type(get_closest_species(pes=pes, indexes=(0, 0))) is ReactantComplex
    assert type(get_closest_species(pes=pes, indexes=(1, 1))) is ReactantComplex

    pes2 = PES2d(reactant=reactant,
                 r1s=np.linspace(1, 5, 5), r1_idxs=(0, 1),
                 r2s=np.linspace(1, 5, 5), r2_idxs=(2, 3))

    with pytest.raises(NoClosestSpecies):
        get_closest_species(pes=pes2, indexes=(4, 4))
