...
def old_minimise_ring_energy(atoms, dihedrals, close_idxs, r0, ring_idxs,
    ...
    # Non-bonded ring atoms used to calculate the repulsion
    ring_pair_idxs = ring_atom_pairs(ring_idxs)
    ...
    # Finally, set the new coordinates
...
def ring_atom_pairs(ring_idxs):
    """
    Generate i, j pairs (atom indexes) between non-bonded pairs in the ring
    used to calculate the repulsive energy

    Arguments:
         ring_idxs (list(int)): Atom indexes that form the ring
...
def dihedral_rotations(angles, coords, dihedrals, close_idxs, r0, rep_idxs):
    ...
    it's current value, where X, Y are the two atoms in the tuple idx_pair
...
def harm_energy(idx_pair, coords, r0):
    """
    E_harmonic = (r_ij - r_0)^2

    Arguments:
        idx_pair (list(int)): Atom indexes for the closure
    ...
    Returns:
        (float):
    """
    idx_i, idx_j = idx_pair
    return (np.linalg.norm(coords[idx_i] - coords[idx_j]) - r0)**2
