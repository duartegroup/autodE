...
class CompositePrimitive(Primitive):
    """Linear combination of multiple primitive coordinates"""

    def __init__(self, *args: Primitive, coeffs):
        """
        Initialise from multiple primitives, such as bond lengths,
        angles etc. and coefficients for
        ...
        """
        super().__init__()
        assert all(isinstance(arg, Primitive) for arg in args)
        self._prims = list(args)
        self._coeffs = np.array(coeffs)
        if len(self._prims) != len(self._coeffs):
            raise ValueError(
                "Number of primitives must match the number of"
    ...
    def _evaluate(
        ...
        """Values are calculated elsewhere"""
        raise NotImplementedError

    def __call__(self, x: "CartesianCoordinates") -> float:
        """Calculate the value of the composite primitive"""
        val = 0.0
        for idx, prim in enumerate(self._prims):
            val += self._coeffs[idx] * prim(x)
    ...
    def derivative(
        ...
        """
        Calculate the derivative of the composite primitive
        against the Cartesian coordinates
        """
        _x = x.ravel()
        derivs = np.zeros_like(_x, dtype=float)
        for idx, prim in enumerate(self._prims):
            derivs += self._coeffs[idx] * prim.derivative(_x)
        return derivs
    ...
    def second_derivative(
        ...
        """
        Calculate the second derivative of the composite primitive
        against the Cartesian coordinates
        """
        _x = x.ravel()
        x_n = len(_x)
        second_derivs = np.zeros(shape=(x_n, x_n), dtype=float)
        for idx, prim in enumerate(self._prims):
            second_derivs += self._coeffs[idx] * prim.second_derivative(_x)
        return second_derivs

    def __eq__(self, other):
        return isinstance(other, self.__class__) and (
            self._prims == other._prims and self._coeffs == other._coeffs
        )


class CompositeConstrainedPrimitive(CompositePrimitive, ConstrainedPrimitive):
    """Linear combination of coordinates with a constraint"""

    def __init__(self, *args, coeffs, value):
        """
        Constrain this composite coordinate to a certain value
