...
class BITSSOptimiser(BaseOptimiser):
    ...
    def calculate(
        ...
        logger.error(f"Finished optimisation run - converged: {self.converged}")
...
class BinaryImagePair:
    ...
    def bitss_coords(self, value: CartesianCoordinates) -> None:
        ...
        if isinstance(value, np.ndarray):
            assert (value.shape == (2 * 3 * self._left_image.n_atoms,),
                    "Provided array does not have the right shape")
        else:
            raise ValueError(f"bitss_coords only accepts arrays but {type(value)}"
                             " was provided")
