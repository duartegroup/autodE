...
def calc_multiplicity(molecule):
    ...
    """
    n_radical_electrons = rdkit.Chem.Descriptors.NumRadicalElectrons(molecule.rdkit_mol_obj)
...
def init_organic_smiles(molecule, smiles):
    ...
    if not are_coords_reasonable(coords=molecule.get_coordinates()):
        ...
        make_graph(molecule, rdkit_bonds=molecule.rdkit_mol_obj.GetBonds())
    ...
    logger.info(f'Initialisation with SMILES successful. Charge={molecule.charge}, Multiplicity={molecule.mult}, '
                f'Num. Atoms={molecule.n_atoms}')
    return None
