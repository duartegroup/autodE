...
def get_fbonds_bbonds_2b(reactant, product, possible_bond_rearrangs, all_possible_bbonds, all_possible_fbonds, possible_fbond_and_bbonds, bbond_atom_type_fbonds, fbond_atom_type_bbonds, n_isomorphisms):
    ...
    if len(all_possible_bbonds) == 1:
        possible_bbonds = all_possible_bbonds[0]
        logger.info(
            f'Have {int(0.5*len(possible_bbonds)*(len(possible_bbonds)-1))} isomorphisms to do')
        for i in range(len(possible_bbonds)):
            for j in range(len(possible_bbonds)):
                if i > j:
                    possible_bond_rearrangs = add_bond_rearrangment(possible_bond_rearrangs, reactant, product, fbonds=[
        ...
        logger.info(
            f'Have {len(possible_bbonds1)*len(possible_bbonds2)} isomorphisms to do')
...
def get_fbonds_bbonds_1b2f(reactant, product, possible_bond_rearrangs, all_possible_bbonds, all_possible_fbonds, possible_fbond_and_bbonds, bbond_atom_type_fbonds, fbond_atom_type_bbonds, n_isomorphisms):
    ...
    if len(all_possible_bbonds) == 1 and len(all_possible_fbonds) == 2:
        ...
        logger.info(
            f'Have {len(possible_bbonds)*len(possible_fbonds1)*len(possible_fbonds2)} isomorphisms to do')
        for fbond1 in possible_fbonds1:
            for fbond2 in possible_fbonds2:
                for bbond in possible_bbonds:
                    possible_bond_rearrangs = add_bond_rearrangment(
                        possible_bond_rearrangs, reactant, product, fbonds=[fbond1, fbond2], bbonds=[bbond])

    elif len(all_possible_bbonds) == 1 and len(all_possible_fbonds) == 1:
        possible_bbonds = all_possible_bbonds[0]
        possible_fbonds = all_possible_fbonds[0]
        logger.info(
            f'Have {int(len(possible_bbonds)*0.5*len(possible_fbonds)*(len(possible_fbonds)-1))} isomorphisms to do')
        for bbond in possible_bbonds:
            for i in range(len(possible_fbonds)):
                for j in range(len(possible_fbonds)):
                    if i > j:
                        possible_bond_rearrangs = add_bond_rearrangment(possible_bond_rearrangs, reactant, product, fbonds=[
                                                                        possible_fbonds[i], possible_fbonds[j]], bbonds=[bbond])

    elif len(all_possible_bbonds) == 0 and len(all_possible_fbonds) == 1:
        possible_fbonds = all_possible_fbonds[0]
        isomorphisms_to_do = (len(fbond_atom_type_bbonds)*0.5*len(possible_fbonds)*(
            len(possible_fbonds)-1)) + (len(possible_fbonds * n_isomorphisms))
...
def get_fbonds_bbonds_2b1f(reactant, product, possible_bond_rearrangs, all_possible_bbonds, all_possible_fbonds, possible_fbond_and_bbonds, bbond_atom_type_fbonds, fbond_atom_type_bbonds, n_isomorphisms):
    ...
    if len(all_possible_bbonds) == 2 and len(all_possible_fbonds) == 1:
        ...
        possible_bbonds2 = all_possible_bbonds[1]
        logger.info(
            f'Have {len(possible_fbonds)*len(possible_bbonds1)*len(possible_bbonds2)} isomorphisms to do')
        for bbond1 in possible_bbonds1:
            for bbond2 in possible_bbonds2:
                for fbond in possible_fbonds:
                    possible_bond_rearrangs = add_bond_rearrangment(
        ...
        possible_fbonds = all_possible_fbonds[0]
        logger.info(
            f'Have {int(len(possible_fbonds)*0.5*len(possible_bbonds)*(len(possible_bbonds)-1))} isomorphisms to do')
        for fbond in possible_fbonds:
            for i in range(len(possible_bbonds)):
                for j in range(len(possible_bbonds)):
                    if i > j:
                        possible_bond_rearrangs = add_bond_rearrangment(possible_bond_rearrangs, reactant, product, fbonds=[
                                                                        fbond], bbonds=[possible_bbonds[i], possible_bbonds[j]])

    elif len(all_possible_bbonds) == 1 and len(all_possible_fbonds) == 0:
        possible_bbonds = all_possible_bbonds[0]
        isomorphisms_to_do = (len(bbond_atom_type_fbonds)*0.5*len(possible_bbonds)*(
            len(possible_bbonds)-1)) + (len(possible_bbonds * n_isomorphisms))
        logger.info(f'Have {int(isomorphisms_to_do)} isomorphisms to do')
...
def get_fbonds_bbonds_2b2f(reactant, product, possible_bond_rearrangs, all_possible_bbonds, all_possible_fbonds, possible_fbond_and_bbonds, bbond_atom_type_fbonds, fbond_atom_type_bbonds, n_isomorphisms):
    ...
    if len(all_possible_bbonds) == 2 and len(all_possible_fbonds) == 2:
        ...
        possible_bbonds2 = all_possible_bbonds[1]
        logger.info(
            f'Have {len(possible_fbonds1)*len(possible_fbonds2)*len(possible_bbonds1)*len(possible_bbonds2)} isomorphisms to do')
        for bbond1 in possible_bbonds1:
            for bbond2 in possible_bbonds2:
                for fbond1 in possible_fbonds1:
                    for fbond2 in possible_fbonds2:
        ...
        possible_fbonds = all_possible_fbonds[0]
        logger.info(
            f'Have {int(len(possible_bbonds1)*len(possible_bbonds2)*0.5*len(possible_fbonds)*(len(possible_fbonds)-1))} isomorphisms to do')
        for bbond1 in possible_bbonds1:
            for bbond2 in possible_bbonds2:
                for i in range(len(possible_fbonds)):
                    for j in range(len(possible_fbonds)):
                        if i > j:
                            possible_bond_rearrangs = add_bond_rearrangment(possible_bond_rearrangs, reactant, product, fbonds=[
                                                                            possible_fbonds[i], possible_fbonds[j]], bbonds=[bbond1, bbond2])
        ...
        possible_fbonds2 = all_possible_fbonds[1]
        logger.info(
            f'Have {int(len(possible_fbonds1)*len(possible_fbonds2)*0.5*len(possible_bbonds)*(len(possible_bbonds)-1))} isomorphisms to do')
        for fbond1 in possible_fbonds1:
            for fbond2 in possible_fbonds2:
                for i in range(len(possible_bbonds)):
                    for j in range(len(possible_bbonds)):
                        if i > j:
                            possible_bond_rearrangs = add_bond_rearrangment(possible_bond_rearrangs, reactant, product, fbonds=[
                                                                            fbond1, fbond2], bbonds=[possible_bbonds[i], possible_bbonds[j]])

    elif len(all_possible_bbonds) == 1 and len(all_possible_fbonds) == 1:
        possible_bbonds = all_possible_bbonds[0]
        possible_fbonds = all_possible_fbonds[0]
        isomorphisms_to_do = (0.5*len(possible_fbonds)*(len(possible_fbonds)-1)*0.5*len(possible_bbonds)*(
            len(possible_bbonds)-1)) + (len(possible_bbonds) * len(possible_fbonds) * n_isomorphisms)
        logger.info(f'Have {int(isomorphisms_to_do)} isomorphisms to do')
                ...
                for j in range(len(possible_fbonds)):
                    if i > j:
                        for bbond2 in fbond_atom_type_bbonds:
                            possible_bond_rearrangs = add_bond_rearrangment(possible_bond_rearrangs, reactant, product, fbonds=[
                                                                            possible_bbonds[i], possible_bbonds[j]], bbonds=[bbond1, bbond2])
                            ...
                            if p > o:
                                for q in range(len(possible_fbond_and_bbonds[m][1])):
                                    for r in range(len(possible_fbond_and_bbonds[m][1])):
                                        if q > r:
                                            bbond1, bbond2 = possible_fbond_and_bbonds[m][
                                                0][o], possible_fbond_and_bbonds[m][0][p]
                                            fbond1, fbond2 = possible_fbond_and_bbonds[m][
                                                1][q], possible_fbond_and_bbonds[m][1][r]
                                            possible_bond_rearrangs = add_bond_rearrangment(
                                                possible_bond_rearrangs, reactant, product, fbonds=[fbond1, fbond2], bbonds=[bbond1, bbond2])
