...
def get_substitution_centres(reactant, bond_rearrangement, shift_factor):
    """Get all the substitution centers in a molecule. A substitution centre is defined as atom that upon reaction
    has a bond made and broken simultaneously

    Arguments:
        reactant (autode.complex.ReactantComplex):
        bond_rearrangement (autode.bond_rearrangement.BondRearrangement):
        shift_factor (float): The multiplier in the ideal A--C distance where A is an attacking atom and C a
                              substitution centre
    """
...
def attack_cost(reactant, subst_centres, attacking_mol_idx, a=1.0, b=1.0, c=10.0, d=1.0):
    ...
    for subst_centre in subst_centres:
        ...
        if len(a_nn_coords) == 0:
            # The attacking atom has no nearest neighbours thus take the attack vector to be a unit vector
            v_ann = np.array([1.0, 0.0, 0.0])
        else:
            v_ann = -np.average(np.array(a_nn_coords), axis=0)

        if length(v_ann) < 1E-1:
            # Attacking atom is planar. Compute the perpendicular from two nearest neighbours
        ...
        cost += c * (1 + np.dot(v_ca, v_cx) / (length(v_ca) * length(v_cx)))        # c(1 + cos(φ))
